...
/*
  v5 — Novidades em relação à v3:
  - Fix: USER_CODE/USER_NAME agora são restaurados a partir do sync ANTES do
    "return" do estado de intervenção. Antes, se o ESP32 reiniciasse com uma
    intervenção aberta, USER_CODE ficava vazio e o stop_intervention (que
    exige usuário válido) era sempre rejeitado pelo servidor — a intervenção
    ficava travada, sem forma de fechar pelo teclado.
  - Fix: mensagem de erro do stop_intervention não é mais apagada pelo sync
    periódico (que redesenhava a cada ~2s mesmo sem o usuário ter feito nada).
  - Fix: debounce adicionado ao '#' que fecha intervenção (única tecla que
    não tinha, até então).
  - Novo: atualização de firmware via OTA por navegador, em /update — não
    precisa mais de cabo USB para atualizar cada máquina.
  - Novo: suporte a HTTPS embutido (flag USE_HTTPS, hoje fixa em false —
    continua em HTTP puro até virar seleção na página de config).
  - Novo: pinout do teclado atualizado para o "padrão antigo" (verificar
    fiação física antes de gravar — comentário de GPIOs no código não bate
    com os valores do array, vale conferir com o hardware real).
  - Mudança: payloads JSON passaram de StaticJsonDocument<1024> (stack) para
    DynamicJsonDocument (heap) — libera espaço de stack, necessário para
    comportar o WiFiClientSecure (~36KB) quando HTTPS for ligado.
*/
...
#include <Keypad.h>
#include <WiFi.h>
#include <WiFiClientSecure.h>   // necessário para HTTPS (usado só se USE_HTTPS for true)
...
// ============================================================
// FLAG DE PROTOCOLO
// true  = HTTPS (usa WiFiClientSecure com setInsecure)
// false = HTTP  (conexão sem criptografia, para rede local)
// Hoje fixo em false — quando a seleção HTTP/HTTPS for exposta na página de
// config e persistida no config.txt, isso deixa de ser #define e vira uma
// variável carregada pelo loadConfig().
// ============================================================
...
// Credenciais da página de atualização de firmware via OTA (/update).
// IMPORTANTE: troque por uma senha própria antes de gravar nas máquinas.
...
// Enquanto o sync falhar (validateConfigs == false), o display alterna a
// cada message_interval entre a última mensagem de erro e o SSID do
// hotspot sozinho na tela — sem isso, quem for fazer manutenção fica sem
// saber em qual rede ESP32_XXXXXX conectar enquanto o erro fica preso.
String lastSyncErrorMessage = "";
unsigned long lastSyncDisplayMillis = 0;
bool syncErrorShowingSsid = false;
...
// Padrão antigo
byte pin_rows[ROW_NUM]      = {13, 12, 14, 27}; // GPIO19, GPIO18, GPIO5, GPIO17 conectam nas linhas
byte pin_column[COLUMN_NUM] = {26, 25, 33, 32}; // GPIO16, GPIO4, GPIO0, GPIO2 conectam nas colunas
  ...
  String page = R"rawliteral(
    ...
    <head>
      ...
      <style>
          ...
          button, .btn { padding: 10px 20px; background-color: #28a745; color: white; border: none; cursor: pointer; display: inline-block; text-decoration: none; }
    ...
    <body>
      ...
      <br>
      <a class="btn" href=")rawliteral" + String("/update") + R"rawliteral(">Firmware Update (OTA)</a>
    </body>
    </html>)rawliteral";
...
// ---------------------------------------------------------------------------
// MARK: OTA — Atualização de firmware via navegador (sem precisar de cabo USB)
//
// Acesse http://<ip-do-esp32>/update (na rede WiFi ou no hotspot do próprio
// ESP32), informe otaUsername/otaPassword quando o navegador pedir, escolha
// o .bin gerado no Arduino IDE/PlatformIO (Sketch > Export Compiled Binary)
// e envie. O ESP32 reinicia sozinho já com o novo firmware.
// ---------------------------------------------------------------------------
...
// Chamado pelo WebServer a cada pedaço do arquivo .bin recebido
void handleOtaUpload() {
  ...
  if (upload.status == UPLOAD_FILE_START) {
    ...
    // Autentica no início do upload — se não autenticar aqui, nada é
    // escrito na flash (evita gravar antes de checar a senha).
    otaAuthorized = server.authenticate(otaUsername, otaPassword);

    if (!otaAuthorized) {
      Serial.println("OTA: tentativa de upload sem autenticação bloqueada.");
      return;
    }

    Serial.println("OTA: upload iniciado - " + upload.filename);

    if (!Update.begin(UPDATE_SIZE_UNKNOWN)) {
      Update.printError(Serial);
    }
  }
  else if (upload.status == UPLOAD_FILE_WRITE && otaAuthorized) {
    if (Update.write(upload.buf, upload.currentSize) != upload.currentSize) {
      Update.printError(Serial);
    }
  }
  else if (upload.status == UPLOAD_FILE_END && otaAuthorized) {
    if (Update.end(true)) {
...
// Chamado uma única vez, depois que todo o corpo do POST /update já foi lido
void handleOtaResult() {
  ...
  if (Update.hasError()) {
    server.send(200, "text/plain", "Falha na atualizacao! Veja o Serial do ESP32 para detalhes.");
  } else {
...
// ============================================================
// sendApiRequest — suporta HTTP e HTTPS via flag USE_HTTPS.
//
// HTTPS: aloca WiFiClientSecure no heap (new/delete) para evitar
//        stack overflow, pois o objeto ocupa ~36 KB.
//        Usa setInsecure() para ignorar validação de certificado.
//
// HTTP:  usa http.begin(url) direto, sem cliente explícito.
// ============================================================
...
void handleSyncWorkStation(ApiResult& result) {
    ...
    if (result.httpCode < 0) {
        ...
        lastSyncErrorMessage = "SERVER \n NOT FOUND!";

        // Só desenha e reseta a alternância na TRANSIÇÃO pra erro — se já
        // estava falhando, quem cuida do display é a alternância no loop().
        // Sem essa guarda, o retry de 500ms sempre reseta o timer de 3s
        // antes dele completar, e o SSID nunca chega a aparecer.
        if (previousValidateState) {
            drawCenteredText(lastSyncErrorMessage.c_str(), 1);
            lastSyncDisplayMillis = millis();
            syncErrorShowingSsid = false;
        }
    ...
    if (!msg) {
        validateConfigs = false;
        lastSyncErrorMessage = "Invalid response";
    ...
    // USER_CODE/USER_NAME vêm de CurrentWorkStationParameters e são sempre
    // enviados pelo servidor, independente de haver intervenção aberta.
    // Precisam ser restaurados ANTES do "return" abaixo — senão, depois de
    // um reboot do ESP32 com intervenção aberta, USER_CODE fica "" e o
    // stop_intervention (que exige um usuário válido) é rejeitado pelo
    // servidor, deixando a intervenção travada sem forma de fechar.
    ...
    if (!obj["intervention_data"].isNull()) {
        ...
        // Só redesenha a mensagem de sync se já passou o message_interval —
        // evita apagar, a cada ~2s, um erro que handlestopIntervention acabou
        // de mostrar (ex: falha ao tentar fechar a intervenção).
        if (millis() - previousMessageMillis > message_interval) {
            drawCenteredText(msg, 1);
            previousMessageMillis = millis();
        }
...
void loop() {
  ...
  if (!validateConfigs) {
      ...
      // Alterna a cada message_interval (3s) entre a mensagem de erro e o
      // SSID sozinho na tela, independente da cadência do retry acima.
      if (currentMillis - lastSyncDisplayMillis >= message_interval) {
          lastSyncDisplayMillis = currentMillis;
          syncErrorShowingSsid = !syncErrorShowingSsid;

          if (syncErrorShowingSsid) {
              drawCenteredText(apSSID, 1);
          } else {
              drawCenteredText(lastSyncErrorMessage.c_str(), 1);
          }
      }
  ...
  if (currentWorkStationState == INTERVENTION) {
    if (key == '#' && currentMillis - previousDebounceMillis >= debounce_keypad) {
