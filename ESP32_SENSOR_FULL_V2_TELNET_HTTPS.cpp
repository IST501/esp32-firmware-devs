...
ApiResult sendApiRequest(JsonDocument& payload, const String& route) {
  ...
  if (ENABLE_HTTPS == "1") {
      ...
      if (client) {
          client->setInsecure();
          beginSuccess = http.begin(*client, requestUrl);
          delete client; // Liberar memória após o uso, memory leak
          // O objeto 'client' será limpo automaticamente ou via delete se necessário, 
          // mas para simplificar o fluxo de memória no ESP32:
      }
  } else {
      WiFiClient client;
      beginSuccess = http.begin(client, requestUrl);
