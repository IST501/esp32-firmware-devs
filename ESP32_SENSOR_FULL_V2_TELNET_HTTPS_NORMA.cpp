...
String ENABLE_HTTPS = "0"; // Flag para habilitar HTTPS
...
void saveConfig(String param, String value) {
    ...
    file.println("ENABLE_HTTPS=" + ENABLE_HTTPS);
...
void loadConfig() {
    ...
    if (!file) {
      ...
      saveConfig("ENABLE_TELNET", ENABLE_TELNET);
      saveConfig("ENABLE_HTTPS", ENABLE_HTTPS);
    ...
    while (file.available()) {
      ...
      else if (line.startsWith("ENABLE_HTTPS=")) {
        ENABLE_HTTPS = line.substring(13);
      }
    }
    ...
    sysPrintln("ENABLE_HTTPS: " + ENABLE_HTTPS);
}
    ...
    <body>
      ...
      <h2>Wi-Fi Settings</h2>
          ...
          <label style="font-size: 12px; font-weight: normal; margin: 0; display: inline-block; cursor: pointer;">
              <input type="checkbox" style="width: auto; margin-right: 5px;" onclick="document.getElementById('pwd').type = this.checked ? 'text' : 'password'"> Show Password
          </label><br><br>
      ...
      <h2>Production Hub Settings</h2>
          ...
          <label>Protocol (Security):</label><br>
          <select name="enable_https">
            <option value="0" )rawliteral" + (ENABLE_HTTPS == "0" ? "selected" : "") + R"rawliteral(>HTTP (Normal)</option>
            <option value="1" )rawliteral" + (ENABLE_HTTPS == "1" ? "selected" : "") + R"rawliteral(>HTTPS (Secure)</option>
          </select><br><br>
...
void handleSave() {
  ...
  String newEnableHttps  = server.arg("enable_https");
  ...
  if (newSSID.length() > 0 && newPass.length() > 0 && newWorkStation.length() > 0 && newServerIp.length() > 0 && newServerPort.length() > 0) {
      ...
      saveConfig("ENABLE_HTTPS",  newEnableHttps);
...
ApiResult sendApiRequest(JsonDocument& payload, const String& route) {
  ...
  if (ENABLE_HTTPS == "1") {
        ...
        beginSuccess = http.begin(secureClient, requestUrl);
    } else {
        beginSuccess = http.begin(insecureClient, requestUrl);
    }
